...
@dataclass
class Pact:
    sponsor:Address; steward:Address; place:str; goals:str; protocol:str; amount:u256; status:str; baseline:str; observations:str; outcome:str; unmet:str; rationale:str
...
class CanopyPact(gl.Contract):
    ...
    def fund_pact(self,i:str,steward:str,place:str,goals:list[str],protocol:str,baseline_urls:list[str])->None:
        ...
        if value<=0 or len(gs)<2 or len(clean(protocol,1000))<50 or len(urls)<2:raise gl.vm.UserError(f'{EXPECTED} funded pact, goals, protocol and baseline required')
        if any(not x.startswith('https://') for x in urls):raise gl.vm.UserError(f'{EXPECTED} HTTPS baseline required')
        self.pacts[key]=Pact(gl.message.sender_address,Address(steward),clean(place,300),json.dumps(gs),clean(protocol,1000),u256(value),'FUNDED',json.dumps(urls),'[]','','[]','')
        self.ids.append(key)
    ...
    def _review(self,p:Pact,observations:list[str])->dict:
        baseline=json.loads(p.baseline); all_urls=baseline+observations
        def run()->dict:
            records=[]
            for url in all_urls:
                ...
                records.append(clean(res.body.decode('utf-8'),2200))
            ...
            return {'outcome':out,'unmet':unmet,'rationale':clean(data.get('rationale'),450)}
        def validate(leader:gl.vm.Result)->bool:
            ...
            return mine['outcome']==theirs.get('outcome') and mine['unmet']==theirs.get('unmet')
    ...
    def submit_observations(self,i:str,observation_urls:list[str])->None:
        ...
        if len(urls)<2 or any(not x.startswith('https://') for x in urls):raise gl.vm.UserError(f'{EXPECTED} two HTTPS observations required')
        result=self._review(p,urls);out=result['outcome'];p.observations=json.dumps(urls);p.outcome=out;p.unmet=json.dumps(result['unmet']);p.rationale=result['rationale']
    ...
    def get_pact(self,i:str)->dict:
        p=self._get(i);return {'id':i,'sponsor':p.sponsor.as_hex,'steward':p.steward.as_hex,'place':p.place,'goals':json.loads(p.goals),'protocol':p.protocol,'grant_wei':str(int(p.amount)),'status':p.status,'baseline':json.loads(p.baseline),'observations':json.loads(p.observations),'outcome':p.outcome,'unmet_goal_indexes':json.loads(p.unmet),'rationale':p.rationale}
    @gl.public.view
